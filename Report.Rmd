  ...
  for (dataset in datasets) {
    ...
    Mmax = lm(
      formula(paste(
        ...
        Reduce(
          function (l, r) paste(l, r),
          Map(function (term) paste("-", term), interactionTerms),
          ""
        )
      )),
      data = dataset
...
curve(dnorm(x, mean(births$wt), sd(births$wt)),
      add = TRUE,
      lwd = 3, 
      col = "darkblue")
```
...
```{r test}
dataset = craftFeatures(mendedBirths)
Mmax <- lm(wt ~ (. - gestation - smoke - time - med - fed - income- marital - meth - feth - mBMICategory - htDiff - lateMom)^2, data = dataset)
...
plot(basicBirthWeightModel, cex = 0.4, which = c(1, 2), pch = 19, lwd = 2, main = NULL)
```
...
```{r plotDiagnostic2}
layout(matrix(c(1, 2, 4, 5), 1, 2, byrow = TRUE))
plot(engineeredBirthWeightModel, cex = 0.4, which = c(1, 2), pch = 19, lwd = 2, main = NULL)
```
...
colnames(PMSETable) <- c("Basic Mode", "Engineered Model")
...
Another reason we did not remove these variables was because of the percentage of the population that fell under those categories. For example, the number of people who Quit But Didn't Know When was very small: only 0.4% (See Appendix 3.3.6). This results in a large p-value as their results are less representative of the population than that of a more significant sample.

## Regression Assumptions

Looking at Figure 2.3, we see that the Residual vs Fitted plot has a relatively horizontal line, indicating that the linearity of data assumption has been preserved. We also see that the residuals roughly follow a Normal distribution from the Q-Q Plot.  
The Scale-Location Plot shows a horizontal line, slightly curved downward. Furthermore, the spread of the points is close to equal, with there being slightly more points above the line. Overall, the homoscedasticity of variance assumption is preserved. See Appendix 3.3.5.
...
### Table 3.3 - Basic Model Coefficients
...
### Table 3.4 - Engineered Model Coefficients
```{r pValue2, eval = TRUE}
