...
isNormal <- function (bmi) bmi < normalOverweightBoundary & bmi >= underweightNormalBoundary
isOverweight <- function (bmi) bmi < overweightObeseBoundary & bmi >= normalOverweightBoundary
...
```{r 1}
pairs(births[c("wt", "mht", "mwt","time", "number")], col=adjustcolor("firebrick", 0.2), pch=19, main= "Pairs Plot")
```
...
```{r 2}
hist(births$wt, breaks=100, col = "firebrick", main = "Frequency of Birth Weights", xlab = "Birth Weight (ounces)") 
```
...
```{r 3}
plot(births$meth, births$wt, col=adjustcolor("firebrick", 0.5), pch=16, main = "Mother's Ethnicity vs Birth Weight", xlab = "Mother's Ehtnicity", ylab = "Brith Weight (ounces)") 
```
...
```{r 4}
plot(births$parity, births$wt, pch=16, col="firebrick", main = "Parity vs Birth Weight", xlab = "Parity", ylab = "Birth Weight (ounces)") 
...
```{r 5}
plot(births$med, births$wt, col=adjustcolor("firebrick", 0.5), pch=16, main = "Mother's Ethnicity vs Birth Weight", xlab = "Mother's Ethnicity", ylab = "Birth Weight (ounces)")
plot(births$fed, births$wt, col=adjustcolor("firebrick", 0.5), pch=16, main = "Father's Ethnicity vs Birth Weight", xlab = "Father's Ethnicity", ylab = "Birth Weight (ounces)")
plot(births$mage, births$wt, pch=16, col="firebrick", main = "Mother's Age vs Birth Weight", xlab = " Mother's Age", ylab = "Birth Weight (ounces)") 
plot(births$income, births$wt, pch=16, col="firebrick", main = "Household Income vs Birth Weight", xlab = "Household Income (USD)", ylab = "Birth Weight (ounces)") 
plot(births$med, births$fed, col=adjustcolor("firebrick", 0.5), pch=16, main = "Mother's Ethnicity vs Father's Ethnicity", xlab = "Mother's Ethnicity", ylab = "Father's Ethnicity")
```
...
plot(births$mht, births$fht, col=adjustcolor("firebrick", 0.25), pch=16, main = "Mother's Height vs Father's Height", xlab = "Mother's Height (inches)", ylab = "Father's Height (inches)")
abline(coef=c(0, 1))
...
```{r diffInHeight}
plot(births$fht - births$mht, births$wt, pch=16, col="firebrick", main = "Difference in Parental Height vs Birth Weight", xlab = "Difference in Parental Height (inches)", ylab = "Birth Weight (ounces)")
```
...
```{r BMI}
plot(births$mwt / I(births$mht)^2, births$wt, pch=16, col="firebrick", main = "Mother's BMI vs Birth Weight", xlab = "Mother's BMI (kg/m^2)", ylab = "Birth Weight (ounces)") 
```
...
```{r trial}
plot(births$gestation, births$wt, main = "Gestation Period vs Birth Weight", xlab = "Gestation Period (days)", ylab = "Birth Weight (ounces)")
...
Model 1 is constructed using raw data with decoded categorical datasets. The automated selection process uses stpewise selection.
...
hbar <- p/n
lev.ind <- h > 2*hbar 
# leverage more than 2x the average
...
cex <- .8
plot(h, D, xlab = "Leverage", ylab = "Cook's Influence Measure", main = "Leverage Plot for Basic Model", pch = 21, bg = clrs, cex = cex, cex.axis = cex)
...
D <- cooks.distance(engineeredBirthWeightModel) 
#flag some of the points
infl.ind <- which.max(D) # top influence point
h <- hatvalues(engineeredBirthWeightModel)
p <- length(coef(engineeredBirthWeightModel))
n <- nobs(engineeredBirthWeightModel)
hbar <- p/n
lev.ind <- h > 2*hbar 
# leverage more than 2x the average
clrs <- rep("black", len = n)
...
par(mfrow = c(1,1), mar = c(4,4,1,1))
cex <- .8
plot(h, D, xlab = "Leverage", ylab = "Cook's Influence Measure", main = "Leverage Plot for Engineered Model", pch = 21, bg = clrs, cex = cex, cex.axis = cex)
...
Looking at the PRESS statstic and the AIC of both models, AIC is favouring the Basic Model, while the PRESS statstic favours the Engineered data. However the PRESS statstic has a larger relative difference in the two models compared to AIC, so overall the Engineered Model is preferred using these statstical analysis
...
par(mfrow=c(2,2))
plot(cvResults$residuals$actual, cvResults$residuals$model1, main = "Actual Residuals vs Basic Model Residuals", xlab = "Actual Residuals", ylab = "Basic Model Residuals")
plot(cvResults$residuals$actual, cvResults$residuals$model2,  main = "Actual Residuals vs Engineered Model Residuals", xlab = "Actual Residuals", ylab = "Engineered Model Residuals")

plot(cvResults$residuals$model1Predictions, cvResults$residuals$model1,  main = "Predicted Values vs Basic Model Residuals", xlab = "Predicted Values", ylab = "Basic Model Residuals")
plot(cvResults$residuals$model2Predictions, cvResults$residuals$model2, main = "Predicted Values vs Engineered Model Residuals", xlab = "Predicted Values", ylab = "Engineered Model Residuals")
```
...
#residualPlot(basicBirthWeightModel, main= "Residuals vs Fitted")
#residualPlot(engineeredBirthWeightModel, main = "Residuals vs Fitted")
```

Then, by plotting the Q-Q Plots of residuals of the predicted values vs actual residuals, we found that both models showed evidence of Normality, but for lighter babies, erred on the side of over-predicting their weight, and for heavier babies, erred on the side of under-predicting their weight.
...
```{r qq2}
qqnorm(cvResults$residuals$model1, main = "Normal Q-Q Plot for Basic Model")
abline(a = 0, b = 20, lty = 2, col = "blue")

qqnorm(cvResults$residuals$model2, main = "Normal Q-Q Plot for Engineered Model")
abline(a = 0, b = 20, lty = 2, col = "blue")
```
...
Produces a pair plot of the variables birthweight, mother height, mother weight, time (since quitting smoking), and number (number of cigarettes smoke per day while smoking)
```{r ref.label = "1"}
```
...
Plots the birthweight against the difference in height of the parents
```{r ref.label = "diffInHeight"}
```

### Figure 1.8

Plots the birthweight against the BMI of the mother
```{r ref.label = "BMI"}
```
