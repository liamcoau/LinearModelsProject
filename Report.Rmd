...
We found that the babies' weight had a positive relationship with the mothers' weight as well as with the mothers' height. We also see that the mothers' weight has a positive relationship with the mothers' height.
...
engineeredBirthWeightModel <- selectModel(
  ...
  excludedVars = c("gestation", "smoke", "time", "fht", "mBMICategory", "income",
                   "marital", "med", "fed", "meth", "feth", "fht", "fwt"),
  specialTerms = c("mBMICategory", "time", "income", "marital", "med", "fed", 
                   "feth", "meth", "fht", "fwt")
)
...
plot(basicBirthWeightModel, cex = 0.4, pch = 19, main = NULL)
```
...
```{r plotDiagnostic2}
layout(matrix(c(1, 2, 4, 5), 1, 2, byrow = TRUE))
plot(engineeredBirthWeightModel, cex = 0.4, pch = 19, main = NULL)
```
...
The Scale-Location Plot shows a horizontal line, slightly curved downward. Furthermore, the spread of the points is close to equal, with there being slightly more points above the line. Overall, the homoscedasticity of variance assumption is preserved. See Appendix Figure 3.4.
...
format(basicCoeff, digits = 2, scientific = FALSE)
format(enginCoeff, digits = 2, scientific = FALSE)
```
