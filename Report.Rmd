...
Looking at the data, we see that the distribution of the babies' weight is close to Normal, but has spikes in frequency in certain areas. This likely corresponds to rounding and measurement discrepancies.
...
However, based on the data, there was is no or little correlation between only the mothers' ethnicity and the babies' weight.
...
Based on the data, we see that there there is minimal correlation between only babies' weight and parity.
...
However, we see that the tested variables: mothers' education level, fathers' education level, mothers' age  at the termination of pregnancy, or household income have a very low or no correlation with the babies' weight.
...
Looking at the heights of the mothers and the fathers, we see that majority of the couples had fathers who were taller than the mothers. It is also worth nothing that the mean baby weight when the father is taller than the mother is greater than the mean baby weight when the mother is taller than the father. This suggests a possible correlation between the difference in heights.
...
BMI, as a relatively good indicator of health, would theoretically also be a good indicator of health of a newborn baby. We want to look at the mothers' BMI to see whether that would have a relationship with the baby's newborn weight.
...
Based on the data, there appears to be a small positive correlation between the mothers' BMI and the babies' weight.


We did not consider gestation as a factor of birth weight because it was not something that the couple could know at the time of conception of the baby. 
...
We see that 40% of the entries in both father's weight and father's height are NA, as well as 10% of the entries in income. Without father height, father weight, and income, the percentage of the original data considered would be reduced to 90.2%.
...
Model 1 is constructed using raw data with decoded categorical datasets. The automated selection process uses stepwise selection with direction set to both.
...
We wanted to create a better combined measure of how the mother's smoking habits impact the pregnancy. We hypothesized that there existed a "dimishing returns" effect on the number of cigarettes smoked. (https://www.med.uottawa.ca/sim/data/Smoking_and_Health_e.htm) demonstrates that it took a six-fold increase in the number of cigarettes smoked a day to double the increase of the likelihood multiplier of developing lung cancer. This is interpreted as evidence of a non-linear relationship, so we selected a logarithm of the number of cigarettes smoked as our measure of general health impact.

Secondly, we consider how the health impact fades after quitting smoking. We hypothesized that the health impact would decay exponentially over time as the body recovers. We construct our smoking impact feature as the logarithm of the number of cigarettes smoked per day while smoking, decayed by the number of years since quitting (or preserved if smoking has continued or mother quit during pregnancy). This is parameterized by the "effect half-life" for the decay which had to be selected. We expect that the health impact on the mother of her smoking habits which this is supposed to represent will have an impact on the health of the pregnancy and the weight of the baby.
...
summaryTable <- data.frame(c(basicRes, enginRes), c(basicRsquared, enginRsquared), c(basicARsquared, enginARsquared))
...
By comparing the Residual Standard Error in both the Basic and Engineered Model, we see that the difference between them is not very large. However, we do see that the Residual Standard Error for the Engineered Model is preferable to that of the Basic Model. Similarly, the difference between the R Squared values for both models do not differ by a large amount,but the R Squared value for the Engineered Model is more preferable than that of the Basic Model.
...
Looking at the Residual and Fitted plot, both models do not exhibit any specific trend in the points, hence suggesting that there is a linear relationship present. Similar to the Residual plot, the Normal Q-Q plot also shows strong Normality. However, there is no significant evidence that one model exhibits better results than the other, hence additional diagnostics are required.
...
PMSETable <- data.frame(c(mean(rPMSEs$model1), sd(rPMSEs$model1)), c(mean(rPMSEs$model2), sd(rPMSEs$model2)))
...
plot(
  ...
  main = "Predicted Values vs Basic Model Res",
  cex.main = 0.8,
...
plot(
  ...
  main = "Predicted Values vs Engineered Model Res",
  cex.main = 0.8,
  col = adjustcolor("firebrick", 0.5),
  xlab = "Predicted Values",
...
Then, by plotting the Q-Q Plots of residuals of the predicted values vs actual residuals, we found that both models showed evidence of Normality, but for lighter babies, erred on the side of over-predicting their weight, and for heavier babies, erred on the side of under-predicting their weight. See Appendix Figure 1.9.
...
qqnorm(
  cvResults$residuals$model1,
  main = "Normal Q-Q Plot for Basic",
  cex.main = 0.85,
  pch = 16,
  col = adjustcolor("firebrick", 0.5)
)
...
qqnorm(
  cvResults$residuals$model2,
  main = "Normal Q-Q Plot for Engineered",
  cex.main = 0.85,
  pch = 16,
  col = adjustcolor("firebrick", 0.5)
)
...
Comparing the different diagnostics of the two models, it is evident that the statistical diagnostic of the two models is very similar. However, by taking a look at the residual standard error, coefficients of determination, and PRESS statistic, the Engineered Model is a slightly better model compared to the Basic Model. However it is a lot more computationally complex than the basic model, but since we are only considering the model's predictive power in this course, the Engineered Model will be used for the discussion portion.

# Discussion

## Significance

Looking at the p-values of the Engineered Model in Appendix 3.3.4, the most important factors that affects a baby's birth weight is the time since the mother quit smoking, the mother's ethnicity, the mother's height, the mBMI, and the parity, with the most significant being the mother's ethinicity and height. 

## Behavioural Recommendations

By comparing the different p-value and beta of the coefficients of the Engineered model, some behaivourals changes that the parents could make to have a higher birth weight are:
* Refrain from smoking during pregenancy to lower the smoking impact
* Maintain a higher BMI

## High p-values

Looking at Table 3.3, we see that there are many factors associated with a relatively high p-value: the mother's ethnicity when she's Mexican, Mixed or Other and the time since the mother quit smoking if she Still Smokes, if she Quit Less Than a Year Ago, 2-3 Years Ago, 3-4 Years Ago, 5-9 Years Ago or she Quit But Doesn't Know When.

There were two reasons not to remove these variables. For example, when the mother is Mixed, the p-value is very high. However, we cannot remove the mother's ethnicity as a factor because that would remove other significant covariates, such as the mother's ethnicity when she is African-American.

Another reason we did not remove these variables was because of the percentage of the population that fell under those categories. For example, the number of people who Quit But Didn't Know When was very small: only 0.4% (See Appendix 3.3.5). This results in a large p-value as their results are less representative of the population than that of a more significant sample.

## Regression Assumptions

Looking at Appendix 3.3.4, we see that the Residual vs Fitted plot has a relatively horizontal line, indicating that the linearity of data assumption has been preserved. We also see that the residuals roughly follow a Normal distribution from the Q-Q Plot.  
The Scale-Location Plot shows a horizontal line, slightly curved downward. Furthermore, the spread of the points is close to equal, with there being slightly more points above the line. Overall, the homoscedasticity of variance assumption is preserved. See Appendix 3.3.4.
...
Produces a plot of mother's ethnicity against birth weight. Appears in Data Exploration.
```{r ref.label = "3"}
```

### Figure 1.4

Produces a plot of parity against birth weight. Appears in Data Exploration.
```{r ref.label = "4"}
```

### Figure 1.5.{1, 2, 3, 4}

Produces plots of mother's education level, father's education level, mother's age at the termination of the pregnancy and household income against birth weight. Appears in Data Exploration. 
```{r ref.label = "5"}
```

### Figure 1.6.{1, 2}

Produces a plot of the birth weight against the difference in height of the parents. Appears in Data Exploration.
```{r ref.label = "6"}
```
...
Produces a table of the mean and standard deviations for each model's rPMSE, a boxplot comparison of both rPMSEs, and plots residuals vs fitted values for each model. Appears in Cross Validation.
...
### Table 3.2 - Basic Model Coefficients
...
### Table 3.3 - Engineered Model Coefficients
...
Produces a residuals vs fitted plot and a Normal Q-Q Plot for the Engineered Model. Appears in Discussion.
```{r summaryPlots2, eval = TRUE}
plot(engineeredBirthWeightModel, cex = 0.4, which = c(1, 3), pch = 19, lwd = 2, main = NULL)
```
...
Executes cross validation to compare candidate models. Appears in Cross Validation.
